...
export const FilesView: React.FC<FilesViewProps> = ({ mode = 'full' }) => {
  ...
  const readFileStat = React.useCallback(async (path: string): Promise<FileStatSnapshot | null> => {
    ...
    const resolvedPath = typeof (result as { path?: string }).path === 'string' ? (result as { path: string }).path : path;
    const size = typeof (result as { size?: number }).size === 'number' ? (result as { size: number }).size : 0;
    const mtimeMs = typeof (result as { mtimeMs?: number }).mtimeMs === 'number' ? (result as { mtimeMs: number }).mtimeMs : undefined;
    return {
      path: resolvedPath,
      size,
      mtimeMs,
